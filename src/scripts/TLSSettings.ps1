...
# Contacts: candacej,roguruna
#***************************************************************************************************************

#************************************************ SCRIPT USAGE  ************************************************
# .\TLSSettings.ps1
#   -SetCipherOrder         :   Excellence/Min-Bar, default(Min-Bar), use " " to set excellence. (Min-Bar ordering prefers ciphers with smaller key sizes to improve performance over security)
...
Function DisableRC4 { 
    ...
    Write-Log -Message "----- Checking the status of RC4 -----"  
    ...
    if ($ciphers.SubKeyCount -eq 0) { 
        ...
        Write-Log -Message "RC4 was disabled "  
        $RC4 = $true
    } 
   
    If ($RC4 -ne $true) {
        Write-Log -Message "There was no change for RC4 "  
    }
...
Function Test-RegistryValueForFipsSettings { 
    ...
    foreach ($path in $fipsPath) {
        ...
        Write-Log -Message "Checking to see if $($path)\$fipsValue exists"  
        ...
        Try {
            ...
            if ($result -eq 4) {
                ...
                Write-Log -Message "Regkey $($path)\$fipsValue was changed from value $result to a value of 3"  
                $restart = $true
            }
			else {
                Write-Log -Message "Regkey $($path)\$fipsValue left at value $result"  
			}
        ...
        Catch {
            Write-Log -Message "Error of type $($Error[0].Exception.GetType().FullName) trying to get $($path)\$fipsValue"  
        }
...
function Get-BaseCipherSuitesWin10Above()
{
    ...
    if ($EnableOlderTlsVersions)
    {
        $cipherorder += "TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA"
        $cipherorder += "TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA"
...
############################## Start of functions that revert the changes done by this script in TLS12OnlyMode ###############

#******************* FUNCTION resests the MinEncryptionLevel setting to 4  ***********************
Function ResetFipsSettings { 
    ...
    foreach ($path in $fipsPath) {
        ...
        Try {
            ...
            $result = Get-ItemProperty -Path $path | Select-Object -ExpandProperty $fipsValue
            if ($result -eq 3) {
                set-itemproperty -Path $path -Name $fipsValue -value 4
                Write-Log -Message "Regkey $($path)\$fipsValue was changed from value $result to a value of 4"  
                $restart = $true
            }
			else {
                Write-Log -Message "Regkey $($path)\$fipsValue left at value $result"  
...
#******************* FUNCTION THAT REMOVES ANY RC4 SETTINGS CREATED BY THIS SCRIPT - USED FOR REVERTING THE EFFECT OF SCRIPT*********************** 
Function EnableRC4 { 
   
    $restart = $false
    Write-Log -Message "EnabledRC4: Cleaning up any RC4 keys that may be present. "  
    ...
    $RC4 = $false
    foreach ($path in $RC4Paths){
        if(Test-Path -Path $path)
        {
            Write-Log -Message "----- Found RC4 path $path - Removing it-----"  
    ...
    if ($RC4 -eq $true) {
        Write-Log -Message "Found RC4 keys and cleaned up. Machine needs to be rebooted"  
    }
    $restart 
} 
################################# End of functions that revert the changes done by this script in TLS12OnlyMode ###############


#*****************************Create the logfile if not does not exist*************************************** 
$logLocation = CreateLogDirectory  
Write-Log -Message "========== Start of logging for a script execution =========="  


#check if TLS 1.2 only mode needs to be set. 
$enableTLS12Only = $false
$isEmulated = $false 
$transportSecuritySetting = [Environment]::GetEnvironmentVariable("TLSLevel")
$emulationSetting = [Environment]::GetEnvironmentVariable("EMULATED")

if($transportSecuritySetting -ne $null)
{
    Write-Log -Message "Environment variable 'TLSLevel' is set to: $transportSecuritySetting"  
    if([string]::Compare($transportSecuritySetting, "TLS12Only", $true) -eq 0)
    {
        $enableTLS12Only = $true
        Write-Log -Message "Environment variable ''TLSLevel' is set to enable TLS 1.2 only. Proceeding with setting TLS 1.2 settings."  
    }
}
else
{
    Write-Log -Message "Environment variable 'TLSLevel' is NOT set. We will skip all TLS 1.2 Only mode."  
}

# checking if we are running in a devfabric 
if($emulationSetting -ne $null)
{
    Write-Log -Message "Environment variable 'Emulated' is set to: $emulationSetting"  
    if([string]::Compare($emulationSetting, "true", $true) -eq 0)
    {
        $isEmulated = $true
        Write-Log -Message "Environment variable 'Emulated' is set to enable 'true'. skip TLS settings. "  
    }
}
else
{
    Write-Log -Message "We are not running in dev fabric. We will set TLS settings based on the value of TLSLevel."  
}
...
if($true)
{
    ...
    if ($EnableOlderTlsVersions)
    {
        ...
        Write-Log -Message "Enabling TLS1.2, TLS1.1, TLS1.0. Disabling SSL3.0, SSL2.0"  
    }
    else
    {
        $registryPathBadGuys += Get-RegKeypathForTls10
        $registryPathBadGuys += Get-RegKeyPathForTls11
        Write-Log -Message "Enabling TLS1.2. Disabling TLS1.1, TLS1.0, SSL3.0, SSL2.0"  
    }


    Write-Log -Message "Check which registry keys exist already and which registry keys need to be created."   
    ...
    For ($i = 0; $i -lt $registryPathGoodGuys.Length; $i = $i + 1) { 
	       ...
	       If (!(Test-Path -Path $registryPathGoodGuys[$i])) { 
            New-Item $registryPathGoodGuys[$i] | Out-Null
     	      Write-Log -Message "Creating key: $($registryPathGoodGuys[$i]) "  
 	      }
    } 
    ...
    For ($i = 0; $i -lt $registryPathBadGuys.Length; $i = $i + 1) { 
        ...
        Write-Log -Message "Checking for existing of key: $($registryPathBadGuys[$i]) "  
	       If (!(Test-Path -Path $registryPathBadGuys[$i])) { 
            Write-Log -Message "Creating key: $($registryPathBadGuys[$i]) "  
    ...
    For ($i = 0; $i -lt $registryPathBadGuys.Length; $i = $i + 1) {
        ...
        if ($registryPathBadGuys[$i].Contains("Client") -Or $registryPathBadGuys[$i].Contains("Server")) {
            ...
            Write-Log -Message "Disabling this key: $($registryPathBadGuys[$i]) "  
    ...
    For ($i = 0; $i -lt $registryPathGoodGuys.Length; $i = $i + 1) {
        ...
        if ($registryPathGoodGuys[$i].Contains("Client") -Or $registryPathGoodGuys[$i].Contains("Server")) {
            ...
            Write-Log -Message "Enabling this key: $($registryPathGoodGuys[$i]) "   
    ...
    Write-Log -Message "----- starting ciphersuite order calculation -----"   
    ...
    if ($SetCipherOrder.ToUpper() -eq "B")
    {
        ...
        Write-Log -Message "The min bar cipher suite order was chosen."  
    }
    else
    {
        Write-Host "The opportunity for excellence cipher suite order was chosen."
        Write-Log -Message "The opportunity for excellence cipher suite order was chosen."   
    }
    $cipherlist = @()
    ...
    $cipherorder = [System.String]::Join(",", $cipherlist)
     ...
     Write-Log -Message "Appropriate ciphersuite order : $cipherorder"  
    ...
    { 
        ...
        $reboot = $True 
        Write-Log -Message "Creating key: $($CipherSuiteRegKey) "  
    } 
 
    $val = (Get-Item -Path $CipherSuiteRegKey -ErrorAction SilentlyContinue).GetValue("Functions", $null)
    Write-Log -Message "Previous cipher suite value: $val  "  
    Write-Log -Message "New cipher suite value     : $cipherorder  "  		 
	   
    if ($val -ne $cipherorder) 
    { 
        Write-Log -Message "Cipher suite order needs to be updated. "  
        ...
        Write-Log -Message "Cipher suite value was updated. "  
        $reboot = $True 
    }
    else
    {
        Write-Log -Message "Cipher suite order does not need to be updated. "  
	    Write-Log -Message "Cipher suite value was not updated as there was no change. " 
    }
	   	
    #****************************** CHECK THE FIPS SETTING WHICH IMPACTS RDP'S ALLOWED CIPHERS **************************
    #Check for FipsSettings
    Write-Log -Message "Checking to see if reg keys exist and if MinEncryptionLevel is set to 4"  
    $result = Test-RegistryValueForFipsSettings 
    $reboot = $reboot -or $result 
   
}# end of if($enableTLS12Only)
elseif($isEmulated -eq $false)
{
######## This section of the script gets called during a deployment with TLS Leve set to Legacy mode. 
######## Identified the registry keys that needs to be changed by comparing the keys from a role instance where this script was never run to an instance it was run
######## CipherSuiteOrder need not be reset here as the AdminInstaller.exe takes care of this. 

    $registryPathGoodGuys = @()
    $registryPathGoodGuys += Get-RegKeypathForTls10
    $registryPathGoodGuys += Get-RegKeyPathForTls11
    $registryPathGoodGuys += Get-RegKeyPathForTls12
    Write-Log -Message "Enabling TLS1.1 and TLS1.0 if disabled" 
    ...
    For ($i = 0; $i -lt $registryPathGoodGuys.Length; $i = $i + 1) { 
	       ...
	       Write-Log -Message "Checking for existing of key: $($registryPathGoodGuys[$i]) "
	       If (!(Test-Path -Path $registryPathGoodGuys[$i])) { 
            New-Item $registryPathGoodGuys[$i] | Out-Null
     	      Write-Log -Message "Creating key: $($registryPathGoodGuys[$i]) "  
       }
    ...
    # Remove any RC4 keys that may be there. 
    $result = EnableRC4
    $reboot = $reboot -or $result
    $result = ResetFipsSettings
    $reboot = $reboot -or $result
}
...
if ($RebootIfRequired)  
{
    Write-Log -Message "You set the RebootIfRequired flag to true. If changes are made, the system will reboot "  
    # If any settings were changed, reboot 
    If ($reboot) 
    { 
        Write-Log -Message "Rebooting now... "  
        Write-Log -Message "Using this command: shutdown.exe /r /t 5 /c ""Crypto settings changed"" /f /d p:2:4 "  
    ...
    { 
        ...
        Write-Log -Message "Nothing get updated. "  
    }  
}
else
{

    Write-Log -Message "You set the RebootIfRequired flag to false. If changes are made, the system will NOT reboot "  
    Write-Log -Message "No changes will take effect until a reboot has been completed. "  
    Write-Log -Message "Script does not include a reboot by design" 
}
Write-Log -Message "========== End of logging for a script execution =========="  